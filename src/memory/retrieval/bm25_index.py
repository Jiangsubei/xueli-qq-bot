...
class BM25Index:
    ...
    def build_index(self, user_id: str, memories: List[MemoryItem]) -> bool:
        """为指定用户构建 BM25 索引。"""
        if not memories:
            logger.debug("跳过 BM25 构建: user=%s, 原因=无记忆", user_id)
        ...
        try:
            ...
            logger.info("BM25 索引已构建: user=%s, 记忆数=%s", user_id, len(memories))
            return True
        except Exception as e:
            logger.error("BM25 构建失败: user=%s, 错误=%s", user_id, e, exc_info=True)
            return False
    ...
    def search(
        ...
        if not index_data or not index_data.bm25:
            logger.debug("BM25 索引不可用: user=%s", user_id)
            return []
        ...
        try:
            ...
            if not query_tokens:
                logger.debug("BM25 查询为空: user=%s", user_id)
                return []
            ...
            logger.error("BM25 检索失败: user=%s, 错误=%s", user_id, e, exc_info=True)
            return []
    ...
    def get_stats(self, user_id: str) -> Dict:
        ...
        return {
            "status": "ready" if index_data.bm25 else "empty",
    ...
    def invalidate(self, user_id: str):
        ...
        if user_id in self._indices:
            ...
            logger.info("BM25 索引已失效: user=%s", user_id)
