...
class WebUIRuntimeServer:
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            logger.warning('WebUI failed to start, bot will continue: %s', exc)
            self._httpd = None
            self._thread = None
            return False
        ...
        self._thread.start()
        logger.info('WebUI URL: %s', self.display_url)
        return True
...
def create_webui_runtime_server_from_env() -> WebUIRuntimeServer:
    ...
    try:
        ...
        logger.warning('Invalid WEBUI_PORT, fallback to 8000')
        port = 8000
