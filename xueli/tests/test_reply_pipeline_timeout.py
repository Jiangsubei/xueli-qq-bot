...
class ReplyPipelineTimeoutTests(unittest.IsolatedAsyncioTestCase):
    async def test_get_reply_suppresses_timeout_message_when_model_times_out(self) -> None:
        ...
        self.assertEqual(result.source, "error_suppressed")
        self.assertEqual(result.text, "")
        self.assertEqual(result.segments, [])

    async def test_get_reply_suppresses_api_error_message_when_upstream_fails(self) -> None:
        pipeline = ReplyPipeline(_FakeHost())
        event = MessageEvent.from_dict(
            {
                "post_type": "message",
        ...
        async def fake_generate_reply(self, *, event, prepared):
            del event, prepared
            raise AIAPIError('API 请求失败: 429, {"error":{"message":"service overloaded"}}')
